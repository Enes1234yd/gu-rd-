...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: "Ozi System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Ozi System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, {reason: "Ozi System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Ozi System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Ozi System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Ozi System | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Ozi System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Ozi System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Ozi System | Webhook Silindi!"}))
  channel.guild.members.ban(entry.executor.id, {reason: "Ozi System | İzinsiz Webhook Açmak!"})
